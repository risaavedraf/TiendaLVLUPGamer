...
function Modal({ isOpen, onClose, onSave, type, initialData = null }: ModalProps) {
  ...
  useEffect(() => {
    if (selectedRegion) {
      ...
      setAvailableComunas(regionData ? regionData.comunas.map(c => c.nombre) : []);
    } else {
      setAvailableComunas([]);
    }
  ...
  return (
    ...
    <div className="modal-overlay" onClick={onClose} style={styles.overlay}>
      ...
      <div className="modal-content-admin" onClick={(e) => e.stopPropagation()} style={styles.content}>
        <h3 id="modal-title">
          {initialData ? 'Editar' : 'Añadir'} {type === 'user' ? 'Usuario' : type === 'product' ? 'Producto' : 'Orden'}
        </h3>
        
        {/* Renderizado condicional del formulario */}
        {type === 'user' && (
          <div style={styles.formFields}>
            <input type="text" name="run" placeholder="Run" value={formData.run || ''} onChange={handleChange} style={styles.input} />
            <input type="text" name="nombre" placeholder="Nombre" value={formData.nombre || ''} onChange={handleChange} required style={styles.input} />
            <input type="text" name="apellido" placeholder="Apellidos" value={formData.apellido || ''} onChange={handleChange} style={styles.input} />
            ...
            {!initialData && (
              <input type="password" name="contrasenia" placeholder="Contraseña (temporal)" onChange={handleChange} required style={styles.input} />
            )}
            <input type="text" name="direccion" placeholder="Dirección" value={formData.direccion || ''} onChange={handleChange} style={styles.input} />
            {/* Selector de Región */}
            <select name="region" value={selectedRegion} onChange={handleChange} required style={styles.input}>
            ...
            {/* Selector de Comuna */}
            <select name="comuna" value={formData.comuna || ''} onChange={handleChange} required disabled={!selectedRegion} style={styles.input}>
            ...
            {/* Selector de Rol */}
            <select name="rol" value={formData.rol || 'Usuario'} onChange={handleChange} required style={styles.input}>
        ...
        {type === 'product' && (
         ...
         {type === 'order' && (
           <p>La edición/creación de órdenes se manejaría de forma diferente (probablemente con selección de productos).</p>
